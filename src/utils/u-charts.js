  ...
  for (let i = 0; i < series.length; i++) {
    ...
    if (
      ...
      seriesItem.data = item.formatter
        ? item.formatter(item.data[index])
        : item.data[index];
  ...
  var textList = seriesData.map(function (item) {
    ...
    return {
      text: option.formatter
        ? option.formatter(item, titleText[index], index, opts)
        : item.name + ": " + item.data,
      color: item.color,
    };
  });
...
function getPieTextMaxLength(series, config, context) {
  ...
  for (let i = 0; i < series.length; i++) {
    ...
    maxLength = Math.max(
      maxLength,
      measureText(text, config.fontSize, context)
    );
  }
  return maxLength;
}
...
function calTooltipYAxisData(point, series, opts, config, eachSpacing) {
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    item = opts.yAxis.data[i].formatter
      ? opts.yAxis.data[i].formatter(Number(item))
      : item.toFixed(0);
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (item.showLabel) {
      let labelText = opts.yAxis.formatter
        ? opts.yAxis.formatter(Number(item.value))
        : item.value;
      context.setFontSize(config.fontSize);
      let textWidth = measureText(labelText, config.fontSize, context);
  ...
  if (type === "pie" || type === "ring" || type === "rose") {
    config._pieTextMaxLength_ =
      opts.dataLabel === false
        ? 0
        : getPieTextMaxLength(seriesMA, config, context);
  }
  switch (type) {
    case "word":
